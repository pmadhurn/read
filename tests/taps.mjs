...
await page.getByText('Add profile').click(); await page.fill('.modal input[type=text]', 'TapAudit'); await page.click('.modal button[type=submit]'); await page.waitForSelector('.ring');
...
for (const hash of ['#/', '#/library', `#/book/${r.book_id}`, `#/read/${r.book_id}`, '#/upload', '#/ranks', '#/stats', '#/badges', '#/settings', '#/profiles']) {
  ...
  const small = await page.evaluate(() => [...document.querySelectorAll('a, button, input, select, textarea, [tabindex="0"]')].filter((e) => { const r = e.getBoundingClientRect(); const st = getComputedStyle(e); return r.width > 0 && st.visibility !== 'hidden' && !e.classList.contains('sr') && !e.closest('.chart') && (r.width < 24 || r.height < 24); }).map((e) => `${e.tagName}.${e.className} "${(e.textContent || e.getAttribute('aria-label') || '').trim().slice(0, 24)}" ${Math.round(e.getBoundingClientRect().width)}x${Math.round(e.getBoundingClientRect().height)}`));
  bad += small.length; console.log(hash, small.length ? small : 'ok');
}
await page.evaluate(async (id) => { const hd = { 'X-Profile-Id': localStorage.getItem('read.profile') }; await fetch('/api/me', { headers: hd }); }, r.book_id);
