...
| Blink aids (user request 2026-09-24): faint trail of the last five words under the ORP line; blink breaks — a dimmed beat of adjustable length (0.3–3 s, default 1 s) with a countdown line, at the next sentence end every 5/10/20 s; replay-sentence while playing (Left arrow, Backspace, R, ⏪ or a tap on the left edge) that eases back in with the ramp-up; speed also on + − [ ]; full shortcut list in the settings panel. | done | `tests/blink.mjs` (12 checks) |
