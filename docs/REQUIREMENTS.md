...
site through Cloudflare (38 checks), `LOGIC` = `tests/logic.py` (14 checks), `TAPS` = `tests/taps.mjs`,
...
| AC-9 passcode change signs out every device | done | CODE |
...
| LB-4 Continue reading row | done | CODE |
...
| NV-9 dictionary definition | done | CODE |
...
| GM-17 celebration pop-up | done | CODE |
...
| ST-6 compare two profiles | done | CODE |
...
| AD-4 change passcode and PIN | done | CODE |
...
| NF-4 progress local first, synced when online | done | CODE |
...
| NF-9 installable PWA, current book offline | done | CODE |
